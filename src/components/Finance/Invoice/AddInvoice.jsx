...
const AddInvoice = ({ setActivityToggle, activityToggle, handleRefresh, customerDetails }) => {
    ...
    const fetchLeadServices = async (leadId) => {
        try {
            ...
            const formattedData = resData.data.map((item) => ({
                label: `${item?.Service?.name} || Service Cost: ₹${item?.cost} || Pending Cost: ₹${item?.estimateCost}`,
    ...
    useEffect(() => {
        if (customerDetails?.customerId) {
            ...
            setFormData((prevData) => ({
                ...
                customerAddress: customerDetails.leadCity
            }))

        }

        if (customerDetails?.convertedLeadId) {
            const leadId = customerDetails?.convertedLeadId
            fetchLeadServices(leadId)
        }
    ...
    useEffect(() => {
        if (formData?.serviceId) {
            ...
            // console.log("selectedService selectedService selectedService =>", selectedService)
            ...
            setFormData((prevData) => ({
                ...
                Item: [
                    {
                        itemNo: 1,
                        itemName: selectedService?.data?.Service?.name,
                        quantity: 1,
                        price: selectedService?.data?.cost,
                        discount: selectedService?.data?.discountPercentage,
                        total: ''
                    }
                ],
                leadServiceId: selectedService?.data?.id
            }));
        }

    }, [formData?.serviceId, serviceOpitons])
