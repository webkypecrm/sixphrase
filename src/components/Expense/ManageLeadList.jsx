...
import AddLead from '../PaidExpense/AddLead';
...
const ManageLeadList = ({
    ...
    let columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                >
                    ...
                    <ul>
                        <li>{record?.VendorSubCategory?.name || record?.Staff?.name || 'N/A'}</li>
                    </ul>
                </Link >
            }
        },
        ...
        {
            title: "Vendor Name",
            key: "vendorId", // Consider changing if "expenseId" is incorrect
            render: (text, record) => {
                if (!record?.Vendor) return "-"; // Fallback in case Vendor is missing

                return (
                    <Link
                        to={`/sales/vendor-details/${record?.expenseId}`}
                        className="table-avatar d-flex align-items-center"
                        style={{ cursor: "pointer" }}
                    >
                        <span className="company-img menu-list-icon">
                        ...
                        <ul>
                            <li>{record?.Vendor?.vendorName?.slice(0, 15) || "N/A"}</li>
                        </ul>
                    </Link>
                );
            }
    ...
    return (
        <>
            ...
            <AddLead
                ...
                expenseOptions={[
                    {
                        label: leadDetails?.Vendor?.vendorName + " || " + "Total Payable: ₹" + leadDetails?.totalPayable,
                        value: leadDetails?.expenseId,
                        data: leadDetails
                    }
                ]}
                fetchLeadData={() => { return }}
            />
