...
import { fileTypeSelect } from "../../../selectOption/selectOption";
...
const AddTaskDocuments = ({ fetchLeadFollowupData, taskDetails }) => {
  ...
  const handleFileChange = (event) => {
    ...
    if (files.length > 0) {
      ...
      if (
        ...
        attachmentFile.type.startsWith("application/x-zip-compressed")
      ) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/product/file-update`, {
        method: "POST",
        headers: {
