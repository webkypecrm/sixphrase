...
import SearchSection from "../../components/Expense/SearchSection";
const ManageLeadList = lazy(() => import("../../components/Expense/ManageLeadList"));
import AddLead from "../../components/Expense/AddLead";
import ContentLoader from "../../components/Layouts/ContentLoader/Index";
import EditLead from "../../components/Expense/EditLead";
...
const ExpensePage = () => {
    ...
    const fetchLeadData = async (page) => {
        try {
            ...
            let url = `${apiUrl}/expense/expense-list?page=${page ? page : 1}&pageSize=${pageSize}&to=${to}&from=${from} &industry=${industry}&source=${source}&country=${country}&state=${state}&city=${city}&stage=${stage}&leadOwner=${leadOwner}&assignedTo=${assignedTo}&leadFor=${leadFor}&search=${search}&categoryId=${categoryId}`
    ...
    const fetchVendorData = async () => {
        try {
            const response = await axios.get(`${apiUrl}/vendor/vendorsDropdown`, {
                headers: {
                    Authorization: `Bearer ${Token}`
                }
            });
            ...
            <div className="content">
                <div className="row">
                    <div className="col-md-12">
                        {/* Page Header */}
                        <PageHeader title="Total Expenses" count={totalPages} pageRefresh={handleRefreshPage} />
                        {/* /Page Header */}
                        {/* <LeadPipeline
