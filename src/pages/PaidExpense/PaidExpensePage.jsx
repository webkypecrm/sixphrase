...
import SearchSection from "../../components/PaidExpense/SearchSection";
const ManageLeadList = lazy(() => import("../../components/PaidExpense/ManageLeadList"));
import AddLead from "../../components/PaidExpense/AddLead";
import ContentLoader from "../../components/Layouts/ContentLoader/Index";
import EditLead from "../../components/PaidExpense/EditLead";
import Filter from '../../components/PaidExpense/Filter'
import './LeadPage.css';
import { SearchContext } from '../../context/SearchProvider';

const PaidExpensePage = () => {
    ...
    const fetchLeadData = async (page) => {
        try {
            ...
            let url = `${apiUrl}/expense/paid-expense-list?page=${page ? page : 1}&pageSize=${pageSize}&to=${to}&from=${from} &industry=${industry}&source=${source}&country=${country}&state=${state}&city=${city}&stage=${stage}&leadOwner=${leadOwner}&assignedTo=${assignedTo}&leadFor=${leadFor}&search=${search}&categoryId=${categoryId}`
    ...
    const fetchExpenseData = async () => {
        try {
            const response = await axios.get(`${apiUrl}/expense/expenseDropdown`, {
                headers: {
                    Authorization: `Bearer ${Token}`
                }
            });
            const formattedData = response.data.data.map((item) => ({
                label: (item.Vendor?.vendorName || item.Staff?.name) + " || " + "Total Payable: ₹" + item.totalPayable,
                value: item.expenseId,
                data: item
            }));
            setExpenseOptions(formattedData);
            ...
            <div className="content">
                <div className="row">
                    <div className="col-md-12">
                        {/* Page Header */}
                        <PageHeader title="Total Paid Expenses" count={totalPages} pageRefresh={handleRefreshPage} />
                        {/* /Page Header */}
                        {/* <LeadPipeline
...
export default PaidExpensePage