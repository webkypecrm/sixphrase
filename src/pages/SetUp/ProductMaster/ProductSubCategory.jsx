...
const ProductSubCategory = () => {
  ...
  const fetchCategoryData = async () => {
    try {
      ...
      setProductCategoryOptions(formattedData);
    } catch (error) {
      setError(error);
    }
  };
  ...
  const handleAdd = async (event) => {
    ...
    const { name, productCategoryId } = formData;

    if (!name?.trim() || !productCategoryId) {
    ...
    try {
      const response = await fetch(`${apiUrl}/product/addProductSubCategory`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${Token}`,
...
const handleEdit = async (event) => {
    ...
    try {
        const response = await axios.put(
            `${apiUrl}/product/update-productsubcategory`,
            {
                id: formData.id,
                name: formData.name,
  ...
  const handleDelete = async (event) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/product/delete-productsubcategory/${productSubCategory.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${Token}`,
  ...
  key: "productCategory",
  render: (text, record) => (
    <div>{record?.productCategory || "N/A"}</div>
          ...
          <div className="content">
            <div className="row">
              <div className="col-md-12">
                {/* Page Header */}
                <PageHeader title="Product Sub Category" count={totalPages} />
                {/* /Page Header */}
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Add New Product Sub Category</h5>
                <button
                  className="btn-close"
