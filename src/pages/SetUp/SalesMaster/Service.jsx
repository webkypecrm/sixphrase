...
const Service = () => {
    ...
    // console.log('ServiceDetails =>', ServiceDetails, num1++)
    ...
    const handleAdd = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/master/add-service`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${Token}`
                },
    ...
    const handleEdit = async (event) => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/master/edit-service`, {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${Token}`
                },
            ...
            const resData = await response.json();
            if (!response.ok) {
                throw new Error(resData.message || 'Failed to edit lead for');
            }
    ...
    const handleDelete = async (event) => {
        event.preventDefault();
        try {
            const response = await fetch(`${apiUrl}/master/delete-service/${serviceDetails.id}`, {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${Token}`
                }
            ...
            const resData = await response.json();
            if (!response.ok) {
                throw new Error(resData.message || 'Failed to delete lead for');
            }
    ...
    const columns = [
...

        {
            ...
            render: (text, record) => (
                <div>
                    {getDate(record?.createdAt)}{" "}{getTime(record?.createdAt)}
                </div>
            ),
            align: 'center'
        },
                ...
                <div className="dropdown table-action">
                    ...
                    <div className="dropdown-menu dropdown-menu-right">
                        <Link
                            ...
                            onClick={() => {
                                setServiceDetails(record)
                                setFormData((prevData) => ({
                                    ...prevData,
                                    leadForId: record.leadForId,
                                    name: record.name,
                                    id: record.id
                                }))
                            }}
    ...
    return (
        <div>
                ...
                <div className="content">
                    <div className="row">
                        <div className="col-md-12">
                            {/* Page Header */}
                            <PageHeader title="Service" count={totalPages} />
                            {/* /Page Header */}
            ...
            {/* /Page Wrapper */}
            {/* Add New Source */}
            <div className="modal custom-modal fade" id="add_source" Service="dialog">
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title">Add New Services</h5>
                            <button
                                className="btn-close"
                            ...
                            <form onSubmit={handleAdd}>
                                <div className="form-wrap">
                                    ...
                                    <Select
                                        ...
                                        className="select"
                                        value={leadForOptions.find((ele) => formData.leadForId === ele.value)}
                                        onChange={(event) => {
                                            let { value } = event
                                            handleInputChange({ target: { name: 'leadForId', value } })
                                        }}
                                ...
                                <p style={{ fontSize: 'small', color: '#7695FF' }}>Note: You can add multiple Services at once by separating them with commas (,)</p>
