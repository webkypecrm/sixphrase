...
const Category = () => {
  ...
  const handleAdd = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/master/add-category`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to add category');
      }
  ...
  const handleEdit = async (event) => {
    ...
    try {
      ...
      const resData = await response.json();
      if (!response.ok) {
        throw new Error(resData.message || 'Failed to edit category');
      }
  ...
  const handleDelete = async (event) => {
    event.preventDefault();
    try {
      const response = await fetch(`${apiUrl}/master/delete-category/${departmentDetails.id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${Token}`
        }
